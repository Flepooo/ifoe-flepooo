...
export async function registerAction(
  ...
  try {
    ...
    if (!response.ok)
      return actionResponse("Failed to register user", data.status);

    if (data?.data?.token) {
      const cookiesStore = cookies();
      const cookie = (await cookiesStore).set(
        AUTH_COOKIE_NAME,
        data?.data?.token,
        {
...
export async function loginAction(values: {
  ...
  try {
    ...
    if (!response.ok || !data?.data?.token) {
      return actionResponse("Failed to login", response.status);
    }

    // ✅ Explicitly set cookie
    (
      await // ✅ Explicitly set cookie
      cookies()
    ).set({
    ...
    return { status: 200, message: "Login successful", data: data.data };
  } catch (error) {
...
export async function changePersonalInformationAction(
  values: z.infer<typeof UserSchema.Update>
) {
  const token = await getAuthorizationToken();
  ...
  try {
    ...
    if (!response.ok)
      return actionResponse("Failed to update user", data.status);
...
export async function changePasswordAction(
  values: z.infer<typeof UserSchema.Password>
) {
  const token = await getAuthorizationToken();
  try {
    ...
    const data: APIResponse<User> = await response.json();
    if (!response.ok)
      return actionResponse("Failed to update password", data.status);
    return data;
  } catch (error) {
